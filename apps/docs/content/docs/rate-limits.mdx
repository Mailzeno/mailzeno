...
<Callout type="info" title="Forms endpoints">
Public Forms submission endpoints also return the same headers, but use the dedicated `form` limiter scope (per form + client IP).
See [MailZeno Forms](/forms) for endpoint details.
</Callout>
